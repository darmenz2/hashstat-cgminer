                                                    ...
                                                    int request_id)
{
	char request[2048];
	if (!pool->stratum_active ||
	    !hashstat_submit_json_locked(pool, work, request_id, request, sizeof(request) - 1))
		return HS_SHARE_STALE;
	/* Bound the send while the share registry lock is held. The old blocking
	 * send after select could stall every pool; never retry on another session. */
	size_t length = strlen(request), sent = 0;
	request[length++] = '\n';
	struct timespec start, now;
	if (pool->sock < 0 || pool->sock >= FD_SETSIZE || clock_gettime(CLOCK_MONOTONIC, &start))
		return HS_SHARE_FAILED;
	while (sent < length) {
		if (clock_gettime(CLOCK_MONOTONIC, &now)) return HS_SHARE_FAILED;
		int64_t elapsed_us = (int64_t)(now.tv_sec - start.tv_sec) * 1000000 +
		                     (now.tv_nsec - start.tv_nsec) / 1000;
		if (elapsed_us < 0 || elapsed_us >= 1000000) return HS_SHARE_FAILED;
		struct timeval timeout = {0, (suseconds_t)(1000000 - elapsed_us)};
		if (timeout.tv_usec == 1000000) { timeout.tv_sec = 1; timeout.tv_usec = 0; }
		fd_set ready; FD_ZERO(&ready); FD_SET(pool->sock, &ready);
		int selected = select(pool->sock + 1, NULL, &ready, NULL, &timeout);
		if (selected < 0 && errno == EINTR) continue;
		if (selected <= 0) return HS_SHARE_FAILED;
		int flags = MSG_DONTWAIT;
#ifdef MSG_NOSIGNAL
		flags |= MSG_NOSIGNAL;
#endif
		ssize_t n = send(pool->sock, request + sent, length - sent, flags);
		if (n < 0 && (errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK)) continue;
		if (n <= 0) return HS_SHARE_FAILED;
		sent += (size_t)n;
	}
	pool->cgminer_pool_stats.times_sent++;
	pool->cgminer_pool_stats.bytes_sent += sent;
	pool->cgminer_pool_stats.net_bytes_sent += sent;
	return HS_SHARE_SENT;
}
#endif
...
{
	uint32_t mask, low_nibble, remaining;
	unsigned shift = 0;
#ifdef USE_HASHSTAT_AML88
	const uint32_t allowed = HS_CORE_BM1362_MASK;
#else
	/* Legacy table entries are signed int. Until that ABI is migrated, reject
	 * sign-bit masks rather than invoke implementation-defined conversions. */
	const uint32_t allowed = UINT32_C(0x7fffffff);
#endif
	if (!pool || !json_is_string(val) ||
	    !hs_core_parse_vmask(json_string_value(val), json_string_length(val),
	                         allowed, &mask))
		return false;
	remaining = mask;
	while ((remaining & 15U) == 0) {
		remaining >>= 4;
		shift += 4;
	}
	low_nibble = (remaining & 15U) << shift;
	pool->vmask_003[0] = (int)mask;
	pool->vmask_003[1] = (int)(mask - low_nibble);
	pool->vmask_003[2] = (int)low_nibble;
	applog(LOG_INFO, "Pool %d accepted version mask %08x", pool->pool_no,
	       (unsigned)mask);
	return true;
}

#ifdef USE_HASHSTAT_AML88
/* The initial socket has one receiver, before the ordinary receive thread.
 * Every frame and write shares one absolute post-connect deadline. Never call
 * legacy recv_line here: select followed by a blocking partial-line recv can
 * exceed its timeout, and a notification is not an RPC reply. */
#define HS_HANDSHAKE_LINE_MAX 32768U
#define HS_HANDSHAKE_PENDING_MAX 32U
#define HS_HANDSHAKE_BYTES_MAX 131072U
#ifdef HASHSTAT_STRATUM_PARSER_TEST
#define HS_HANDSHAKE_MS 200
#else
#define HS_HANDSHAKE_MS 15000
#endif
struct hs_handshake {
	struct timespec deadline;
	char *pending[HS_HANDSHAKE_PENDING_MAX];
	size_t pending_count, pending_bytes, frames;
};
static bool hs_handshake_start(struct hs_handshake *state)
{
	memset(state, 0, sizeof(*state));
	if (clock_gettime(CLOCK_MONOTONIC, &state->deadline)) return false;
	state->deadline.tv_sec += HS_HANDSHAKE_MS / 1000;
	state->deadline.tv_nsec += (HS_HANDSHAKE_MS % 1000) * 1000000L;
	if (state->deadline.tv_nsec >= 1000000000L) {
		++state->deadline.tv_sec; state->deadline.tv_nsec -= 1000000000L;
	}
	return true;
}
static bool hs_handshake_time(struct hs_handshake *state, struct timeval *remaining)
{
	struct timespec now;
	if (clock_gettime(CLOCK_MONOTONIC, &now)) return false;
	int64_t us = (int64_t)(state->deadline.tv_sec - now.tv_sec) * 1000000 +
	             (state->deadline.tv_nsec - now.tv_nsec) / 1000;
	if (us <= 0) return false;
	remaining->tv_sec = us / 1000000;
	remaining->tv_usec = us % 1000000;
	return true;
}
static bool hs_handshake_wait(struct pool *pool, struct hs_handshake *state, bool writing)
{
	if (pool->sock <= 0 || pool->sock >= FD_SETSIZE) return false;
	for (;;) {
		struct timeval remaining;
		if (!hs_handshake_time(state, &remaining)) return false;
		fd_set ready; FD_ZERO(&ready); FD_SET(pool->sock, &ready);
		int n = select(pool->sock + 1, writing ? NULL : &ready,
		               writing ? &ready : NULL, NULL, &remaining);
		if (n < 0 && errno == EINTR) continue;
		return n > 0;
	}
}
static bool hs_handshake_send(struct pool *pool, struct hs_handshake *state, const char *text)
{
	size_t length = strlen(text), sent = 0;
	if (length > 2046) return false;
	char framed[2048]; memcpy(framed, text, length); framed[length++] = '\n';
	while (sent < length) {
		if (!hs_handshake_wait(pool, state, true)) return false;
		int flags = MSG_DONTWAIT;
#ifdef MSG_NOSIGNAL
		flags |= MSG_NOSIGNAL;
#endif
		ssize_t n = send(pool->sock, framed + sent, length - sent, flags);
		if (n < 0 && (errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK)) continue;
		if (n <= 0) return false;
		sent += (size_t)n;
	}
	...
	pool->cgminer_pool_stats.bytes_sent += sent;
	pool->cgminer_pool_stats.net_bytes_sent += sent;
	return true; /* Deliberately no plaintext authorize/request logging. */
}
static char *hs_handshake_line(struct pool *pool, struct hs_handshake *state)
{
	for (;;) {
		struct timeval remaining;
		if (!hs_handshake_time(state, &remaining) || !pool->sockbuf) return NULL;
		size_t used = strlen(pool->sockbuf);
		if (used > HS_HANDSHAKE_LINE_MAX) return NULL;
		char *newline = strchr(pool->sockbuf, '\n');
		if (newline) {
			size_t length = (size_t)(newline - pool->sockbuf);
			if (!length) return NULL;
			char *line = malloc(length + 1);
			if (!line) return NULL;
			memcpy(line, pool->sockbuf, length); line[length] = 0;
			memmove(pool->sockbuf, newline + 1, used - length);
			pool->cgminer_pool_stats.times_received++;
			pool->cgminer_pool_stats.bytes_received += length + 1;
			pool->cgminer_pool_stats.net_bytes_received += length + 1;
			return line;
		}
		if (used == HS_HANDSHAKE_LINE_MAX || !hs_handshake_wait(pool, state, false)) return NULL;
		char block[2048];
		size_t capacity = HS_HANDSHAKE_LINE_MAX - used;
		if (capacity > sizeof(block)) capacity = sizeof(block);
		ssize_t n = recv(pool->sock, block, capacity, MSG_DONTWAIT);
		if (n < 0 && (errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK)) continue;
		if (n <= 0 || memchr(block, 0, (size_t)n)) return NULL;
		if (pool->sockbuf_size < used + (size_t)n + 1) {
			char *next = realloc(pool->sockbuf, used + (size_t)n + 1);
			if (!next) return NULL;
			pool->sockbuf = next; pool->sockbuf_size = used + (size_t)n + 1;
		}
		memcpy(pool->sockbuf + used, block, (size_t)n);
		pool->sockbuf[used + (size_t)n] = 0;
	}
}
static bool hs_handshake_notification(json_t *frame)
{
	json_t *id = json_object_get(frame, "id"), *method = json_object_get(frame, "method");
	const char *name = json_string_value(method);
	if ((id && !json_is_null(id)) || !name || strlen(name) != json_string_length(method) ||
	    json_object_get(frame, "result")) return false;
	return !strcmp(name, "mining.notify") || !strcmp(name, "mining.set_difficulty") ||
	       !strcmp(name, "mining.set_version_mask");
}
static json_t *hs_handshake_response(struct pool *pool, struct hs_handshake *state,
                                    int id, bool defer)
{
	while (state->frames++ < 64) {
		char *line = hs_handshake_line(pool, state);
		if (!line) return NULL;
		json_error_t error;
		json_t *frame = json_loads(line, JSON_REJECT_DUPLICATES, &error);
		if (!json_is_object(frame)) { free(line); json_decref(frame); return NULL; }
		if (json_object_get(frame, "method")) {
			bool ok = hs_handshake_notification(frame);
			if (ok && defer) {
				size_t length = strlen(line);
				ok = state->pending_count < HS_HANDSHAKE_PENDING_MAX &&
				     length <= HS_HANDSHAKE_BYTES_MAX - state->pending_bytes;
				if (ok) {
					state->pending_bytes += length;
					state->pending[state->pending_count++] = line; line = NULL;
				}
			} else if (ok) ok = parse_method(pool, line);
			free(line); json_decref(frame);
			if (!ok) return NULL;
			continue;
		}
		free(line);
		json_t *rid = json_object_get(frame, "id");
		if (json_is_integer(rid) && json_integer_value(rid) == id) return frame;
		json_decref(frame); return NULL; /* Never accept an unrelated or string ID. */
	}
	return NULL;
}
static void hs_handshake_clear(struct hs_handshake *state)
{
	for (size_t i = 0; i < state->pending_count; ++i) free(state->pending[i]);
}
static bool hashstat_configure_stratum(struct pool *pool, struct hs_handshake *state)
{
	char request[256];
	int id;
	if (!hashstat_next_request_id(&id)) return false;
	snprintf(request, sizeof(request),
	         "{\"id\":%d,\"method\":\"mining.configure\",\"params\":[[\"version-rolling\"],"
	         "{\"version-rolling.mask\":\"1fffe000\",\"version-rolling.min-bit-count\":0}]}", id);
	if (!hs_handshake_send(pool, state, request)) return false;
	json_t *response = hs_handshake_response(pool, state, id, true);
	json_t *rid = json_object_get(response, "id");
	json_t *result = json_object_get(response, "result");
	json_t *failure = json_object_get(response, "error");
	json_t *rolling = json_object_get(result, "version-rolling");
	json_t *mask_value = json_object_get(result, "version-rolling.mask");
	uint32_t mask = 0;
	bool ok = json_is_object(response) && json_is_integer(rid) && json_integer_value(rid) == id &&
	          json_is_object(result) && (!failure || json_is_null(failure));
	if (ok && json_is_true(rolling))
		ok = json_is_string(mask_value) && hashstat_parse_mask_text(json_string_value(mask_value),
		                                                          json_string_length(mask_value), &mask);
	else if (!json_is_false(rolling)) ok = false;
	if (ok) ok = hashstat_apply_configure(pool, json_is_true(rolling), mask);
	json_decref(response);
	return ok;
}
#elif defined(USE_VMASK)
...
#ifdef USE_HASHSTAT_AML88
	/* Validate all fixed/variable fields before legacy allocations/publication.
	 * Fixed bounds also prevent old strlen/alloca/merkle partial-update paths. */
	if (!pool || !json_is_array(val) || json_array_size(val) != 9 ||
	    !json_is_boolean(json_array_get(val, 8))) return false;
	for (size_t field = 0; field < 8; ++field) {
		if (field == 4) continue;
		json_t *v = json_array_get(val, field);
		if (!json_is_string(v) || json_string_length(v) != strlen(json_string_value(v))) return false;
		size_t length = json_string_length(v);
		if (!field) {
			if (!length || length > HS_STRATUM_JOB_ID_MAX || !valid_ascii(json_string_value(v))) return false;
		} else if (field == 2 || field == 3) {
			if (length > 8192 || (length & 1) || (length && !valid_hex(json_string_value(v)))) return false;
		} else if (length != (field == 1 ? 64 : 8) || !valid_hex(json_string_value(v))) return false;
	}
	json_t *branches = json_array_get(val, 4);
	if (!json_is_array(branches) || json_array_size(branches) > 32) return false;
	for (size_t branch = 0; branch < json_array_size(branches); ++branch) {
		json_t *v = json_array_get(branches, branch);
		if (!json_is_string(v) || json_string_length(v) != 64 ||
		    strlen(json_string_value(v)) != 64 || !valid_hex(json_string_value(v))) return false;
	}
#endif
...
#ifdef USE_HASHSTAT_AML88
	size_t bounded_cb = strlen(coinbase1) / 2 + pool->n1_len + pool->n2size + strlen(coinbase2) / 2;
	if (pool->n2size < 2 || pool->n2size > 8 || pool->n1_len > 64 ||
	    bounded_cb < 44 || bounded_cb > 8192 || !pool->nonce1bin ||
	    !hashstat_note_job_locked(pool, job_id, (uint32_t)strtoul(bbversion, NULL, 16),
	                             clean || memcmp(pool->prev_hash, prev_hash, 64))) {
		cg_wunlock(&pool->data_lock);
		free(job_id); free(coinbase1); free(coinbase2);
		goto out;
	}
#endif
...
static bool parse_diff(struct pool *pool, json_t *val)
{
	...
	if (!pool || !json_is_array(val) || json_array_size(val) != 1)
		return false;
	number = json_array_get(val, 0);
	if (!json_is_number(number))
		return false;
	diff = json_number_value(number);
	if (!hs_core_valid_difficulty(diff))
		return false;
	...
	if (old_diff != diff)
		applog(LOG_NOTICE, "Pool %d difficulty changed to %.17g",
		       pool->pool_no, diff);
	else
...
#ifdef USE_HASHSTAT_AML88
	uint32_t mask;
	if (!json_is_array(params) || json_array_size(params) != 1) return false;
	json_t *value = json_array_get(params, 0);
	if (!json_is_string(value) || !hashstat_parse_mask_text(json_string_value(value),
	                                                     json_string_length(value), &mask)) return false;
	return hashstat_apply_mask(pool, mask);
#endif
	if (!params) {
...
#ifdef HASHSTAT_STRATUM_PARSER_TEST
bool hs_test_set_vmask(struct pool *pool, json_t *value) { return set_vmask(pool, value); }
bool hs_test_parse_vmask(struct pool *pool, json_t *value) { return parse_vmask(pool, value); }
bool hs_test_parse_diff(struct pool *pool, json_t *value) { return parse_diff(pool, value); }
#endif
...
{
#ifdef USE_HASHSTAT_AML88
	struct hs_handshake state;
	json_t *request = NULL, *response = NULL;
	char *text = NULL;
	bool ok = false;
	int id;
	if (!hs_handshake_start(&state)) { suspend_stratum(pool); return false; }
	if (!pool->rpc_user || !pool->rpc_pass || strlen(pool->rpc_user) > 128 ||
	    strlen(pool->rpc_pass) > 512 || !hashstat_next_request_id(&id)) goto hs_out;
	request = json_pack("{s:i,s:s,s:[s,s]}", "id", id, "method", "mining.authorize",
	                    "params", pool->rpc_user, pool->rpc_pass);
	if (!request || !(text = json_dumps(request, JSON_COMPACT))) goto hs_out;
	mutex_lock(&pool->stratum_lock);
	ok = pool->stratum_active && hs_handshake_send(pool, &state, text);
	mutex_unlock(&pool->stratum_lock);
	if (!ok) goto hs_out;
	ok = false;
	response = hs_handshake_response(pool, &state, id, false);
	json_t *failure = json_object_get(response, "error");
	if (!json_is_true(json_object_get(response, "result")) ||
	    (failure && !json_is_null(failure))) goto hs_out;
	mutex_lock(&pool->stratum_lock);
	cg_wlock(&pool->data_lock);
	ok = pool->stratum_active && pool->hs_stratum.ready && !pool->hs_stratum.exhausted;
	pool->hs_stratum.authorized = ok;
	cg_wunlock(&pool->data_lock);
	mutex_unlock(&pool->stratum_lock);
	if (ok && opt_suggest_diff) {
		char suggestion[160]; int suggestion_id;
		if (!hashstat_next_request_id(&suggestion_id)) { ok = false; goto hs_out; }
		snprintf(suggestion, sizeof(suggestion), "{\"id\":%d,\"method\":\"mining.suggest_difficulty\",\"params\":[%d]}",
		         suggestion_id, opt_suggest_diff);
		mutex_lock(&pool->stratum_lock);
		ok = pool->stratum_active && hs_handshake_send(pool, &state, suggestion);
		mutex_unlock(&pool->stratum_lock);
	}
hs_out:
	free(text); json_decref(request); json_decref(response); hs_handshake_clear(&state);
	if (!ok) suspend_stratum(pool);
	else { pool->probed = true; successful_connect = true; }
	return ok;
#else
...
#ifdef USE_HASHSTAT_AML88
	int auth_id;
	if (!hashstat_next_request_id(&auth_id)) return false;
	json_t *auth_request = json_pack("{s:i,s:s,s:[s,s]}", "id", auth_id, "method", "mining.authorize",
	                                  "params", pool->rpc_user, pool->rpc_pass);
	if (!auth_request) return false;
	char *auth_text = json_dumps(auth_request, JSON_COMPACT);
	json_decref(auth_request);
	if (!auth_text) return false;
	size_t auth_length = strlen(auth_text);
	if (auth_length >= sizeof(s) - 1) { free(auth_text); return false; }
	memcpy(s, auth_text, auth_length + 1); free(auth_text);
#else
	...
	if (
#ifdef USE_HASHSTAT_AML88
	    !json_is_true(res_val) || !json_is_integer(json_object_get(val, "id")) ||
	    json_integer_value(json_object_get(val, "id")) != auth_id ||
#else
	    !res_val || json_is_false(res_val) ||
#endif
	    (err_val && !json_is_null(err_val)))  {
		char *ss;
	...
	mutex_lock(&pool->stratum_lock);
	cg_wlock(&pool->data_lock);
	pool->hs_stratum.authorized = pool->hs_stratum.ready && !pool->hs_stratum.exhausted;
	ret = pool->hs_stratum.authorized;
	cg_wunlock(&pool->data_lock);
	mutex_unlock(&pool->stratum_lock);
	if (!ret) goto out;
#endif
...
#ifdef USE_HASHSTAT_AML88
		int suggest_id;
		if (!hashstat_next_request_id(&suggest_id)) goto out;
		snprintf(s, sizeof(s), "{\"id\":%d,\"method\":\"mining.suggest_difficulty\",\"params\":[%d]}",
		         suggest_id, opt_suggest_diff);
#else
...
bool initiate_stratum(struct pool *pool)
{
	...
	char request[256], *nonce = NULL, *session = NULL;
	unsigned char *nonce_bin = NULL;
	bool ok = false;
	int id;
	/* DNS/connect/proxy are separate existing setup gates. The absolute
	 * protocol deadline begins only once their socket is established. */
	if (!setup_stratum_socket(pool)) return false;
	if (!hs_handshake_start(&state)) { suspend_stratum(pool); return false; }
	cg_wlock(&pool->data_lock);
	pool->next_diff = pool->diff_after = 0; pool->sdiff = 1;
	cg_wunlock(&pool->data_lock);
	if (!hashstat_configure_stratum(pool, &state) || !hashstat_next_request_id(&id)) goto hs_out;
	snprintf(request, sizeof(request), "{\"id\":%d,\"method\":\"mining.subscribe\",\"params\":[\""PACKAGE"/"VERSION"\"]}", id);
	if (!hs_handshake_send(pool, &state, request)) goto hs_out;
	response = hs_handshake_response(pool, &state, id, true);
	json_t *result = json_object_get(response, "result"), *error = json_object_get(response, "error");
	json_t *nonce_value = json_array_get(result, 1), *size_value = json_array_get(result, 2);
	const char *nonce_text = json_string_value(nonce_value);
	if (!json_is_array(result) || json_array_size(result) != 3 ||
	    !json_is_array(json_array_get(result, 0)) || (error && !json_is_null(error)) ||
	    !nonce_text || json_string_length(nonce_value) != strlen(nonce_text) ||
	    !valid_hex(nonce_text) || strlen(nonce_text) > 128 || (strlen(nonce_text) & 1) ||
	    !json_is_integer(size_value) || json_integer_value(size_value) < 2 ||
	    json_integer_value(size_value) > 8) goto hs_out;
	session = get_sessionid(result);
	if (session && strlen(session) > 128) goto hs_out;
	nonce = strdup(nonce_text);
	nonce_bin = calloc(strlen(nonce_text) / 2, 1);
	if (!nonce || !nonce_bin || !hex2bin(nonce_bin, nonce_text, strlen(nonce_text) / 2)) goto hs_out;
	mutex_lock(&pool->stratum_lock);
	cg_wlock(&pool->data_lock);
	free(pool->sessionid); pool->sessionid = session; session = NULL;
	free(pool->nonce1); pool->nonce1 = nonce; nonce = NULL;
	free(pool->nonce1bin); pool->nonce1bin = nonce_bin; nonce_bin = NULL;
	pool->n1_len = strlen(pool->nonce1) / 2;
	pool->n2size = (int)json_integer_value(size_value);
	pool->nonce2 = 0;
	pool->hs_stratum.ready = pool->hs_stratum.configured && !pool->hs_stratum.exhausted;
	pool->stratum_active = pool->hs_stratum.ready;
	ok = pool->stratum_active;
	cg_wunlock(&pool->data_lock);
	mutex_unlock(&pool->stratum_lock);
	if (ok && !pool->stratum_url) pool->stratum_url = pool->sockaddr_url;
	/* Apply early messages in wire order only after extranonce/ready exist.
	 * Work is still unauthorized and therefore cannot be submitted. */
	for (size_t i = 0; ok && i < state.pending_count; ++i)
		ok = parse_method(pool, state.pending[i]);
hs_out:
	free(nonce); free(nonce_bin); free(session); json_decref(response);
	hs_handshake_clear(&state);
	if (!ok) suspend_stratum(pool);
	return ok; /* No legacy resume/retry: every new socket has a fresh epoch. */
#else
...
#ifdef USE_HASHSTAT_AML88
	int subscribe_id;
	if (!hashstat_next_request_id(&subscribe_id)) goto out;
	/* Never resume HashStat work across a socket epoch, even if the server
	 * returns identical extranonce1/session text. */
	snprintf(s, sizeof(s), "{\"id\":%d,\"method\":\"mining.subscribe\",\"params\":[\""PACKAGE"/"VERSION"\"]}", subscribe_id);
#else
	if (recvd) {
...
rereceive:
	...
	if (!valid_hex(nonce1)
#ifdef USE_HASHSTAT_AML88
	    || strlen(nonce1) > 128 || (strlen(nonce1) & 1) ||
	    !json_is_integer(json_object_get(val, "id")) || json_integer_value(json_object_get(val, "id")) != subscribe_id
#endif
	   ) {
