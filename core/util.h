...
/* Match the definitions in util.c: only the Linux monotonic non-Bitmain
 * branch returns a sleep duration. Darwin/FreeBSD/Windows branches are void. */
#if defined(USE_BITMAIN_SOC) || !defined(CLOCK_MONOTONIC) || defined(__FreeBSD__) || defined(__APPLE__) || defined(WIN32)
