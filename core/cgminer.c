...
bool hashstat_next_request_id(int *request_id)
{
	...
	if (swork_id >= 0 && swork_id < INT_MAX) { *request_id = swork_id++; ok = true; }
...
#else
	(void)pool; /* HashStat uses only the version owned by this work item. */
#endif
	flip64(data32, work->data);
	sha256_init(&ctx);
	sha256_update(&ctx, data, 64);
...
{
	if (!work || !work->pool || !work->hs_snapshot.valid ||
	    !work->hs_snapshot.rolling || (version_bits & ~work->hs_snapshot.mask)) return false;
	uint32_t version = (work->hs_snapshot.base_version & ~work->hs_snapshot.mask) | version_bits;
	work->data[0] = (unsigned char)(version >> 24);
	work->data[1] = (unsigned char)(version >> 16);
	work->data[2] = (unsigned char)(version >> 8);
	work->data[3] = (unsigned char)version;
	work->hs_snapshot.actual_version = version;
...
#ifdef USE_HASHSTAT_AML88
	if (work->stratum && !hashstat_work_current(work)) return true;
#else
...
#ifdef USE_HASHSTAT_AML88
	if (!json_is_integer(id_val) || json_integer_value(id_val) < 0 ||
	    json_integer_value(id_val) > INT_MAX || !json_is_boolean(res_val)) goto out;
#endif
...
#ifdef USE_HASHSTAT_AML88
	if (sshare && sshare->work->pool != pool) sshare = NULL;
	if (sshare) stale_session = sshare->work->hs_snapshot.session_epoch != pool->hs_stratum.session_epoch;
#endif
	if (sshare) {
...
#ifdef USE_HASHSTAT_AML88
	cg_runlock(&pool->data_lock);
	if (stale_session) { free_work(sshare->work); free(sshare); goto out; }
	/* Never fabricate accepted work/difficulty for an untracked response. */
	if (!sshare) goto out;
#endif
	...
	while (42) {
		...
		if (!parse_method(pool, s) && !parse_stratum_response(pool, s))
			...
			if (!gen_stratum_work(pool, work)) {
				free_work(work); free(s); continue;
			}
...
struct hs_sent_key {
	bool valid;
	uint64_t session, job, nonce2;
	size_t n2size;
	unsigned char header[80];
};
/* Owns work on entry. On successful publication only the response/pruner owns
 * it. Never touch either work or sshare after releasing the registry lock. */
static bool hashstat_submit_queued(struct pool *pool, struct work *work, struct hs_sent_key *last)
{
	struct stratum_share *share = NULL;
	enum hs_share_send_result result = HS_SHARE_STALE;
	struct hs_sent_key key = {0};
	int id;
	if (!hashstat_next_request_id(&id)) goto discard;
	share = calloc(1, sizeof(*share));
	if (!share) goto discard;
	share->id = id; share->work = work;
	share->sshare_time = share->sshare_sent = time(NULL);
	mutex_lock(&pool->stratum_lock);
	cg_rlock(&pool->data_lock);
	if (pool->stratum_active && hashstat_work_current_locked(pool, work)) {
		key.valid = true; key.session = work->hs_snapshot.session_epoch;
		key.job = work->hs_snapshot.job_sequence; key.nonce2 = work->nonce2;
		key.n2size = work->nonce2_len; memcpy(key.header, work->data, 80);
		bool duplicate = last->valid && last->session == key.session && last->job == key.job &&
		                 last->nonce2 == key.nonce2 && last->n2size == key.n2size &&
		                 !memcmp(last->header, key.header, 80);
		if (!duplicate) {
			mutex_lock(&sshare_lock);
			HASH_ADD_INT(stratum_shares, id, share); ++pool->sshares;
			result = hashstat_send_share_locked(pool, work, id);
			if (result != HS_SHARE_SENT) { HASH_DEL(stratum_shares, share); --pool->sshares; }
			else *last = key;
			mutex_unlock(&sshare_lock);
		}
	}
	cg_runlock(&pool->data_lock);
	/* Still own S: failed-send cleanup cannot race with installing a new
	 * connection. The helper takes D-write after we dropped D-read/R. */
	if (result == HS_SHARE_FAILED) hashstat_suspend_stratum_locked(pool);
	mutex_unlock(&pool->stratum_lock);
	if (result == HS_SHARE_SENT) return true;
discard:
	free_work(work);
	free(share);
	/* Partial/failed sends are never replayed into a new TCP session. */
	return false;
}
#endif
...
static void *stratum_sthread(void *userdata)
{
	...
	char threadname[16];
#ifdef USE_HASHSTAT_AML88
	struct hs_sent_key last_sent = {0};
#endif
...
#ifdef USE_HASHSTAT_AML88
		if (!hashstat_submit_queued(pool, work, &last_sent)) {
			mutex_lock(&stats_lock);
			++pool->stale_shares; ++total_stale;
			mutex_unlock(&stats_lock);
		}
		continue;
#else
...
static bool gen_stratum_work(struct pool *pool, struct work *work)
{
	...
	cg_wlock(&pool->data_lock);
#ifdef USE_HASHSTAT_AML88
	if (!pool->hs_stratum.ready || !pool->hs_stratum.authorized || pool->hs_stratum.exhausted ||
	    !pool->coinbase || !pool->swork.job_id || !pool->nonce1 || pool->n2size < 2 ||
	    pool->n2size > 8 || pool->nonce2_offset < 0 || pool->coinbase_len < pool->n2size ||
	    pool->nonce2_offset > pool->coinbase_len - pool->n2size) {
		cg_wunlock(&pool->data_lock); return false;
	}
	if (pool->nonce2 == UINT64_MAX ||
	    (pool->n2size < 8 && pool->nonce2 >= (UINT64_C(1) << (8 * pool->n2size)))) {
		cg_wunlock(&pool->data_lock); return false;
	}
#endif
...
#ifdef USE_HASHSTAT_AML88
	if (!hashstat_capture_work_locked(pool, work)) { cg_runlock(&pool->data_lock); return false; }
#endif
	cg_runlock(&pool->data_lock);
...
#if defined(USE_HASHSTAT_AML88) && defined(HASHSTAT_STRATUM_PARSER_TEST)
bool hs_test_gen_stratum_work(struct pool *pool, struct work *work) { return gen_stratum_work(pool, work); }
void hs_test_share_lock_init(void) { mutex_init(&sshare_lock); mutex_init(&stats_lock); }
bool hs_test_submit_queued(struct pool *pool, struct work *work) {
	struct hs_sent_key last = {0}; return hashstat_submit_queued(pool, work, &last);
}
bool hs_test_parse_stratum_response(struct pool *pool, char *text) { return parse_stratum_response(pool, text); }
#endif
...
#ifdef USE_HASHSTAT_AML88
	if (work->stratum && !hashstat_work_current(work)) { free_work(work); return; }
#endif
...
#ifdef USE_HASHSTAT_AML88
	if (work->stratum && !hashstat_work_current(work)) return false;
#endif
...
{
#ifdef USE_HASHSTAT_AML88
	if (argc == 2 && !strcmp(argv[1], "--hashstat-build-info")) {
		puts(HASHSTAT_BUILD_INFO_JSON);
		return 0;
	}
	if (argc == 2 && (!strcmp(argv[1], "--version") || !strcmp(argv[1], "-V"))) {
		puts(HASHSTAT_MINER_NAME " " HASHSTAT_VERSION
		     " (upstream cgminer " HASHSTAT_UPSTREAM_CORE_VERSION "; research, hardware startup unavailable)");
		return 0;
	}
	/* Explicit controller-only bench mode. Never start pool workers, parse
	 * production configuration or probe hardware on this path. */
	if (argc > 1 && !strcmp(argv[1], "--hashstat-standby"))
		return hashstat_supervisor_main(argc, argv);
#endif
	...
	while (42) {
		...
		if (pool->has_stratum) {
			if (opt_gen_stratum_work) {
				if (!gen_stratum_work(pool, work)) { cgsleep_ms(5); continue; }
