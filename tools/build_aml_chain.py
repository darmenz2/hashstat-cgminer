...
LINK = ["transport/runtime/aml_chain.c", "transport/src/aml_uart.c",
        "transport/src/bm1362_integrity.c", "src/job_cache.c", "src/bm1362_rx.c",
        "src/work.c", "src/pow.c", "src/btm_work_wire.c", "src/miner_lifecycle.c"]
NAMES = LINK + ["transport/include/hs_aml_chain.h", "transport/include/hs_aml_uart.h",
    "transport/include/hs_bm1362_integrity.h", "transport/tests/test_aml_chain.c",
    "transport/tests/test_aml_uart.c", "tests/test_job_cache.c", "tests/test_captured_work.c",
    "include/hs_job_cache.h", "include/hs_bm1362_rx.h", "include/hs_work.h",
    "include/hs_pow.h", "include/hs_btm_work_wire.h", "include/hs_miner_lifecycle.h",
    "tools/build_aml_chain.py"]
...
def digest(p):
    if p.is_symlink() or not p.is_file():
        raise ValueError("regular file required: " + str(p))
...
def main():
    ...
    out = Path(tempfile.mkdtemp(prefix="hashstat-chain-build-", dir=args.output_parent))
    receipt = {"schema": "hashstat.aml.chain-build.v1", "sourceSHA256": before,
               "commands": [], "success": False, "hardwareExecuted": False,
               "firmwareInstalled": False, "completeCgminerDriver": False}
    ...
    def run(name, args, timeout=180):
        log = out/(name+".log")
        r = {"name": name, "argv": args, "log": str(log), "returncode": None}
        receipt["commands"].append(r)
        print("BUILD_STAGE", name, flush=True)
        with log.open("xb") as stream:
            p = subprocess.run(args, cwd=out, env=env, stdout=stream,
    ...
    try:
        ...
        run("toolchain", [sys.executable, str(verifier), "--archive",
            str(args.zig_archive), "--directory",
            str(args.zig.parent), "--output", str(out / "toolchain.json"), "--lock", str(args.toolchain_lock)])
        ...
        receipt["toolchainTreeSHA256"] = tool["treeManifestSHA256"]
        snapshot = out/"source"
        for name in NAMES:
            dest = snapshot/name
        ...
        sources = [str(snapshot/name) for name in LINK]
        for label, test in [("chain", "transport/tests/test_aml_chain.c"),
                            ("cache", "tests/test_job_cache.c"),
                            ("capture", "tests/test_captured_work.c")]:
            binary = out/("test-"+label)
            run(label+"-compile", [args.cc]+flags+
                ["-fsanitize=address,undefined", "-fno-sanitize-recover=all", "-fno-omit-frame-pointer"]+sources+
                [str(snapshot/test), "-o", str(binary)])
            run(label+"-native", [str(binary)])
        binary = out/"hashstat-chain-test.arm"
        run("arm-compile", [str(args.zig), "cc", "-target", "arm-linux-musleabihf",
            ...
            ["-ffunction-sections", "-fdata-sections", "-Wl,--gc-sections", "-Wl,--strip-debug",
             "-ffile-prefix-map="+str(snapshot)+"=/usr/src/hashstat/chain"]+sources+
            [str(snapshot/"transport/tests/test_aml_chain.c"), "-o", str(binary)])
        run("elf", [args.readelf, "-h", "-l", "-d", "-A", str(binary)])
        elf = (out/"elf.log").read_text()
        ...
        if "INTERP" in elf or "(NEEDED)" in elf:
            raise ValueError("test binary must be static")
        if before != {name:digest(ROOT/name) for name in NAMES}:
            raise ValueError("source drift during build")
        receipt["binary"] = {"path": str(binary), "bytes": binary.stat().st_size,
                             "sha256": digest(binary), "armExecuted": False}
        ...
        print(json.dumps({"directory":str(out), "success":receipt["success"],
                          "binary":receipt.get("binary")}), flush=True)
